ABM <- function(agents, contacts_list, lambda_list, schedule,
                ...
                steps, step_length_list, testing_rate_list, vaccination_rate_list, agent_presence_list) {
...
#print('vaccinate')
        ...
        if(sum(testing_rate) > 0) {
            ...
            if(max(testing_rate) == 1) {
                ...
                ####
                eligible = ((agents$state %in% c('S', 'E', 'IA', 'IP', 'IM', 'R', 'V1', 'V2', 'V1E', 'V2E')) & !(agents$isolated)) & agent_presence
                indices = indices[eligible[indices]] #this should fix the problem
                #kludging this a little, will discuss in group
